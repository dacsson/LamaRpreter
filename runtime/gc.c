...
extern const size_t
    __start_custom_data __asm("section$start$__DATA$custom_data");
...
void *alloc(size_t size) {
  ...
  printf("Object allocated: content [%p, %p) padding [%p, %p)\n", p,
         p + obj_size, p + obj_size, p + size * sizeof(size_t));
  fflush(stdout);
#endif
  return p;
}
...
// precondition: obj_content is a valid address pointing to the content of an
// object
...
static FILE *print_stack_content(char *filename) {
  ...
  for (size_t *stack_ptr = (size_t *)((void *)__gc_stack_top + 4);
       stack_ptr < (size_t *)__gc_stack_bottom; ++stack_ptr) {
...
static void objects_dfs(FILE *f, void *obj_content) {
  ...
  for (obj_field_iterator field_it = ptr_field_begin_iterator(obj_header);
       !field_is_done_iterator(&field_it); obj_next_field_iterator(&field_it)) {
...
FILE *print_objects_traversal(char *filename, bool marked) {
  ...
  for (int i = 0; i < extra_roots.current_free; i++) {
    fprintf(f, "extra root %p %p: ", extra_roots.roots[i],
...
static void gc_root_scan_stack() {
  for (size_t *p = (size_t *)(__gc_stack_top + sizeof(size_t));
       p < (size_t *)__gc_stack_bottom; ++p) {
...
void mark_phase(void) {
  ...
  fprintf(stderr, "gc_root_scan_stack has started: gc_top=%p bot=%p\n",
          (void *)__gc_stack_top, (void *)__gc_stack_bottom);
#endif
  gc_root_scan_stack();
#if defined(DEBUG_VERSION) && defined(DEBUG_PRINT)
...
void compact_phase(size_t additional_size) {
  ...
  size_t next_heap_size =
      MAX(live_size * EXTRA_ROOM_HEAP_COEFFICIENT + additional_size,
          MINIMUM_HEAP_CAPACITY);
  size_t next_heap_pseudo_size = MAX(next_heap_size, heap.size);

  memory_chunk old_heap = heap;
  heap.begin = mmap(NULL, WORDS_TO_BYTES(next_heap_pseudo_size),
...
size_t compute_locations() {
  ...
  for (; !heap_is_done_iterator(&scan_iter);
       heap_next_obj_iterator(&scan_iter)) {
...
void scan_and_fix_region(memory_chunk *old_heap, void *start, void *end) {
  ...
  for (size_t *ptr = (size_t *)start; ptr < (size_t *)end; ++ptr) {
    ...
    // this can't be expressed via is_valid_heap_pointer, because this pointer
    // may point area corresponding to the old heap
    if (is_valid_pointer((size_t *)ptr_value) &&
        (size_t)old_heap->begin <= ptr_value &&
        ptr_value <= (size_t)old_heap->current) {
      void *obj_ptr =
          (void *)heap.begin + ((void *)ptr_value - (void *)old_heap->begin);
      void *new_addr =
          (void *)heap.begin +
          ((void *)get_forward_address(obj_ptr) - (void *)old_heap->begin);
...
void scan_and_fix_region_roots(memory_chunk *old_heap) {
  ...
  for (int i = 0; i < extra_roots.current_free; i++) {
...
#ifdef DEBUG_PRINT
        fprintf(stderr,
                "|\tskip extra root: %p (%p), since it points to Lama's stack "
                "top=%p bot=%p\n",
                extra_roots.roots[i], (void *)ptr_value, (void *)__gc_stack_top,
                ...
                extra_roots.roots[i] >= (void *)&__start_custom_data)) {
        fprintf(stderr,
                "|\tskip extra root: %p (%p), since it points to Lama's static "
                "area stop=%p start=%p\n",
                extra_roots.roots[i], (void *)ptr_value,
                (void *)&__stop_custom_data, (void *)&__start_custom_data);
        exit(1);
      }
#endif
      else {
    ...
    if ((size_t)old_heap->begin <= ptr_value &&
        ptr_value <= (size_t)old_heap->current) {
      void *obj_ptr =
          (void *)heap.begin + ((void *)ptr_value - (void *)old_heap->begin);
      void *new_addr =
...
void update_references(memory_chunk *old_heap) {
    ...
    if (is_marked(get_object_content_ptr(it.current))) {
      for (obj_field_iterator field_iter = ptr_field_begin_iterator(it.current);
        ...
        void *field_obj_content_addr =
            (void *)heap.begin +
            (*(void **)field_iter.cur_field - (void *)old_heap->begin);
        // important, we calculate new_addr very carefully here, because objects
        // may relocate to another memory chunk
        void *new_addr =
            heap.begin +
            ((size_t *)get_forward_address(field_obj_content_addr) -
             (size_t *)old_heap->begin);
        // update field reference to point to new_addr
        // since, we want fields to point to an actual content, we need to add
        // this extra content_offset because forward_address itself is a pointer
        // to the object's header
        size_t content_offset =
            get_header_size(get_type_row_ptr(field_obj_content_addr));
#ifdef DEBUG_VERSION
  ...
  scan_and_fix_region(old_heap, (void *)__gc_stack_top + sizeof(size_t),
                      (void *)__gc_stack_bottom + sizeof(size_t));
  ...
  scan_and_fix_region(old_heap, (void *)&__start_custom_data,
...
void physically_relocate(memory_chunk *old_heap) {
  ...
  while (!heap_is_done_iterator(&from_iter)) {
    ...
    if (is_marked(obj)) {
      ...
      size_t *to = heap.begin + ((size_t *)get_forward_address(obj) -
                                 (size_t *)old_heap->begin);
...
inline bool is_valid_heap_pointer(const size_t *p) {
  return !UNBOXED(p) && (size_t)heap.begin <= (size_t)p &&
         (size_t)p <= (size_t)heap.current;
}
...
void mark(void *obj) {
  if (!is_valid_heap_pointer(obj) || is_marked(obj)) {
    return;
  }

  // TL;DR: [q_head_iter, q_tail_iter) q_head_iter -- current dequeue's victim,
  // q_tail_iter -- place for next enqueue in forward_address of corresponding
  // element we store address of element to be removed after dequeue operation
  heap_iterator q_head_iter = heap_begin_iterator();
  // iterator where we will write address of the element that is going to be
  // enqueued
  heap_iterator q_tail_iter = q_head_iter;
  queue_enqueue(&q_tail_iter, obj);

  // invariant: queue contains only objects that are valid heap pointers (each
  // corresponding to content of unmarked object) also each object is in queue
  // only once
  while (q_head_iter.current != q_tail_iter.current) {
    ...
    for (obj_field_iterator ptr_field_it = ptr_field_begin_iterator(header_ptr);
      ...
      // if we came to this point it must be true that field_value is unmarked
      // and not currently in queue thus, we maintain the invariant
...
void scan_extra_roots(void) {
  for (int i = 0; i < extra_roots.current_free; ++i) {
    // this dereferencing is safe since runtime is pushing correct pointers into
    // extra_roots
...
void scan_global_area(void) {
  // __start_custom_data is pointing to beginning of global area, thus all
  // dereferencings are safe
  for (size_t *ptr = (size_t *)&__start_custom_data;
       ptr < (size_t *)&__stop_custom_data; ++ptr) {
    mark(*(void **)ptr);
  }
}
#endif
...
void set_extra_roots(size_t extra_roots_size, void **extra_roots_ptr) {
  memcpy(extra_roots.roots, extra_roots_ptr,
         MIN(sizeof(extra_roots.roots), extra_roots_size));
  clear_extra_roots();
}
...
#if defined(DEBUG_VERSION) && defined(DEBUG_PRINT)
    fprintf(stderr,
            "ERROR: get_type_header_ptr: unknown object header, cur_id=%d",
            cur_id);
...
#ifdef DEBUG_PRINT
    fprintf(stderr,
            "ERROR: get_type_header_ptr: unknown object header, ptr is %p, tag "
            "%i, heap size is "
            "%d cur_id=%d stack_top=%p stack_bot=%p ",
            ptr, TAG(*header), heap.size, cur_id, (void *)__gc_stack_top,
            (void *)__gc_stack_bottom);
#endif
...
#ifdef DEBUG_VERSION
    fprintf(stderr,
            "ERROR: obj_size_header_ptr: unknown object header, cur_id=%d",
            cur_id);
    raise(SIGINT); // only for debug purposes
#else
...
size_t sexp_size(size_t members) {
  return get_header_size(SEXP) + MEMBER_SIZE * (members + 1);
}

obj_field_iterator field_begin_iterator(void *obj) {
  lama_type type = get_type_header_ptr(obj);
  obj_field_iterator it = {
      .type = type, .obj_ptr = obj, .cur_field = get_object_content_ptr(obj)};
  switch (type) {
  case STRING: {
...
void obj_next_ptr_field_iterator(obj_field_iterator *it) {
  ...
  } while (!field_is_done_iterator(it) &&
           !is_valid_pointer(*(size_t **)it->cur_field));
}
