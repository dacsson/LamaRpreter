...
typedef size_t ptrt; // pointer type, because can hold a pointer on a
...
#define UNBOXED_TAG                                                            \
    0x00000009 // Not actually a data_header; used to return from LkindOf
#ifdef X86_64
...
typedef struct {
    // store tag in the last three bits to understand what structure this is,
    // other bits are filled with other utility info (i.e., size for array,
    // number of fields for s-expression)
    auint data_header;
    ...
    // last bit is used as MARK-BIT, the rest are used to store address where
    // object should move last bit can be used because due to alignment we can
    // assume that last two bits are always 0's
