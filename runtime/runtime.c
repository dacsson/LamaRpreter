...
extern aint LtagHash(char *s) {
  ...
  while (*p && limit++ < MAX_SEXP_TAGLEN) {
    ...
    if (*q)
      ...
      failure("tagHash: character not found: %c\n", *p);

    p++;
  }

  if (strncmp(s, de_hash(h), MAX_SEXP_TAGLEN) != 0) {
    failure("%s <-> %s\n", s, de_hash(h));
  }
    ...
    case CLOSURE_TAG: {
      ...
      for (i = 0; i < LEN(a->data_header); i++) {
        if (i)
          ...
          printStringBuf("0x%x", (void *)((aint *)a->contents)[i]);
        if (i != LEN(a->data_header) - 1)
          printStringBuf(", ");
      }
      printStringBuf(">");
      break;
  ...
  if (UNBOXED(p))
    ...
    case SEXP_TAG: {
      ...
      if (strcmp(tag, "cons") == 0) {
        ...
        printStringBuf("*** non-list data_header: %s ***", tag);
    } break;
...
extern void *Lsubstring(aint *args /*void *subj, aint p, aint l*/) {
  ...
  failure("substring: index out of bounds (position=%ld, length=%ld, subject "
          "length=%ld)",
          pp, ll, LEN(d->data_header));
  exit(1);
  return NULL;
}
...
void *Lclone(aint *args /*void *p*/) {
  ...
  default:
    failure("invalid data_header %ld in clone *****\n", t);
  }
  pop_extra_root((void **)&args[0]);
...
aint inner_hash(aint depth, auint acc, void *p) {
  ...
  if (UNBOXED(p))
    return HASH_APPEND(acc, UNBOX(p));
    ...
    default:
      failure("invalid data_header %ld in hash *****\n", t);
    }
  ...
  if (UNBOXED(p)) {
    ...
    if (is_valid_heap_pointer(p)) {
      if (is_valid_heap_pointer(q)) {
        ...
        case CLOSURE_TAG:
          COMPARE_AND_RETURN(((void **)a->contents)[0],
                             ((void **)b->contents)[0]);
        ...
        default:
          failure("invalid data_header %ld in compare *****\n", ta);
        }

        for (; i < la; i++) {
          aint c = Lcompare(((void **)a->contents)[i + shift],
                            ((void **)b->contents)[i + shift]);
          if (c != BOX(0))
            return c;
        }
        return BOX(0);
        ...
        return BOX(-1);
    } else if (is_valid_heap_pointer(q))
      return BOX(1);
    else
      return BOX(p - q);
  }
...
extern void *Belem(void *p, aint i) {
  ...
  switch (TAG(a->data_header)) {
  case STRING_TAG:
    return (void *)BOX((char)a->contents[i]);
  case SEXP_TAG:
    return (void *)((aint *)((sexp *)a)->contents)[i];
  default:
    return (void *)((aint *)a->contents)[i];
  }
}
...
extern void *LmakeString(aint length) {
  ...
  r = (data *)alloc_string(
      n); // '\0' in the end of the string is taken into account
...
extern void *Bstring(aint *args /*void *p*/) {
  ...
  strncpy(s_contents, (char *)args[0],
          n + 1); // +1 because of '\0' in the end of C-strings
...
extern aint Btag(void *d, aint t, aint n) {
  ...
  if (UNBOXED(d))
    ...
    r = TO_DATA(d);
    return (aint)BOX(TAG(r->data_header) == SEXP_TAG &&
                     TO_SEXP(d)->tag == UNBOX(t) &&
...
extern aint Bstring_patt(void *x, void *y) {
  ...
  if (UNBOXED(x))
    ...
    if (TAG(rx->data_header) != STRING_TAG)
      return BOX(0);
...
extern aint Lrandom(aint n) {
  ...
  if (UNBOX(n) <= 0) {
    failure("invalid range in random: %ld\n", UNBOX(n));
  }
