...
#define MINIMUM_HEAP_CAPACITY (2048 * 1024)
...
// the only GC-related function that should be exposed, others are useful for
// tests and internal implementation allocates object of the given size on the
// heap
void *alloc(size_t);
...
// makes a snapshot of current objects in heap (both alive and dead), writes
// these ids to object_ids_buf, returns number of ids dumped object_ids_buf is
// pointer to area preallocated by user for dumping ids of objects in heap
...
// scans it and if it meets a pointer, it should be modified in according to
// forward address
void scan_and_fix_region(memory_chunk *old_heap, void *start, void *end);

// takes a pointer to an object content as an argument, returns forwarding
// address
size_t get_forward_address(void *obj);

// takes a pointer to an object content as an argument, sets forwarding address
// to value 'addr'
void set_forward_address(void *obj, size_t addr);

// takes a pointer to an object content as an argument, returns whether this
// object was marked as live
...
// takes a pointer to an object content as an argument, returns whether this
// object was enqueued to the queue (which is used in mark phase)
...
// takes a pointer to an object content as an argument, unmarks object as
// enqueued
...
// returns correct type when pointer to actual data is passed (header is
// excluded)
...
// returns correct object size (together with header) of an object, ptr is
// pointer to an actual data is passed (header is excluded)
...
// returns number of bytes that are required to allocate array with 'sz'
// elements (header included)
size_t array_size(size_t sz);

// returns number of bytes that are required to allocate string of length 'l'
// (header included)
size_t string_size(size_t len);

// returns number of bytes that are required to allocate closure with 'sz-1'
// captured values (header included)
size_t closure_size(size_t sz);

// returns number of bytes that are required to allocate s-expression with
// 'members' fields (header included)
size_t sexp_size(size_t members);

// returns an iterator over object fields, obj is ptr to object header
// (in case of s-exp, it is mandatory that obj ptr is very beginning of the
// object, considering that now we store two versions of header in there)
obj_field_iterator field_begin_iterator(void *obj);

// returns an iterator over object fields which are actual pointers, obj is ptr
// to object header (in case of s-exp, it is mandatory that obj ptr is very
// beginning of the object, considering that now we store two versions of header
// in there)
...
// ptr is pointer to the actual object content, returns pointer to the very
// beginning of the object (header)
