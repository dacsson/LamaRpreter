...
# === 7. test-lama ===
test-lama: build
	@echo "🔍 Running all .lama files in test-lama/ via hex + run..."
	@mkdir -p "$(DUMP_DIR)"
	...
	@for file in test-lama/*.lama; do \
        ...
        if [ -f "$(DUMP_DIR)/$$BASENAME.bc" ]; then \
            echo "✅ Generated bytecode: $(DUMP_DIR)/$$BASENAME.bc"; \
            make run FILE="$(DUMP_DIR)/$$BASENAME.bc"; \
        else \
            echo "❌ Failed to generate .bc for $$file"; \
        fi; \
    done
	@echo "✅ All tests in test-lama/ completed."
